...
export class AnimeService {
    ...
    async setCookieString() {
        const selectedCookies = Object.entries(this.cookies).filter(([name]) => {
            return name.startsWith('BAHARUNE') || name.startsWith('ANIME_SIGN') || name.startsWith('ckM') || name.startsWith('BAHAID') || name.startsWith('BAHAENUR');
        });
        this.headers.Cookie = selectedCookies.map(([name, value]) => `${name}=${value}`).join('; ');
    }
